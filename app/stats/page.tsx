...
import { getStandings, getTeams, getMatches } from '@/lib/api';
    ...
    async function load() {
      ...
      try {
        const [standingsRes, teamsRes, matchesRes] = await Promise.all([
          getStandings(selectedLeague), getTeams(), getMatches(),
        ]);
        if (standingsRes.data?.data) setStandings(standingsRes.data.data.sort((a: any, b: any) => b.points - a.points));
      ...
      <div className="space-y-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <h1 className="text-3xl lg:text-4xl font-bold text-surface-900 dark:text-surface-100 flex items-center gap-3">
            <BarChart3 size={32} className="text-accent-500" />
            Statistics
          </h1>
        ...
        <div className="flex gap-3">
          {['Premier League', 'La Liga'].map((league) => (
            <button key={league} onClick={() => setSelectedLeague(league)} className={`px-5 py-3 rounded-xl text-sm font-semibold transition-all ${selectedLeague === league ? 'bg-accent-500 text-white shadow-lg shadow-accent-500/25' : 'bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-400 hover:bg-surface-200 dark:hover:bg-surface-700'}`}>{league}</button>
          ))}
        </div>
        ...
        {loading && (
          <div className="space-y-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (<div key={i} className="glass-strong rounded-2xl p-6 text-center animate-pulse"><div className="w-12 h-12 bg-surface-200 dark:bg-surface-800 rounded-full mx-auto mb-3" /><div className="h-8 w-16 bg-surface-200 dark:bg-surface-800 rounded mx-auto mb-1" /><div className="h-4 w-20 bg-surface-200 dark:bg-surface-800 rounded mx-auto" /></div>))}
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="glass-strong rounded-2xl p-6 animate-pulse"><div className="h-5 w-32 bg-surface-200 dark:bg-surface-800 rounded mb-4" /><div className="h-64 bg-surface-200 dark:bg-surface-800 rounded" /></div>
              <div className="glass-strong rounded-2xl p-6 animate-pulse"><div className="h-5 w-32 bg-surface-200 dark:bg-surface-800 rounded mb-4" /><div className="h-64 bg-surface-200 dark:bg-surface-800 rounded" /></div>
            </div>
          </div>
        )}
        ...
        {!loading && !error && (
          <>
            <div ref={statsRef} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[{ label: 'Total Teams', value: standings.length },{ label: 'Total Goals', value: totalGoals },{ label: 'Goals/Match', value: avgGoalsPerMatch },{ label: 'Matches', value: totalMatches }].map((stat) => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }} className="glass-strong rounded-2xl p-6">
                  ...
                  return (
                    <div key={team.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-3 bg-surface-100 dark:bg-surface-800 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${width}%` }} transition={{ duration: 0.8, delay: index * 0.1 }} className={`h-full rounded-full ${index === 0 ? 'bg-warning' : index < 4 ? 'bg-accent-500' : 'bg-surface-300 dark:bg-surface-600'}`} />
                      </div>
