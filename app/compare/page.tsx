...
export default function ComparePage() {
  ...
  const getTeamStats = (team: any) => ({
    ...
    cleanSheets: Math.floor(Math.random() * 15) + 2,
    form: ['W', 'D', 'W', 'L', 'W'],
  });
  ...
  const compareBar = (val1: number, val2: number, higherIsBetter = true) => {
    ...
    if (higherIsBetter) {
      return { left: (val1 / max) * 100, right: (val2 / max) * 100 };
    }
      ...
      <div className="space-y-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-surface-500">Loading teams...</p>
          </div>
        )}
        ...
        {!loading && !error && team1Stats && team2Stats && (
          <>
            <div className="grid grid-cols-2 gap-4">
              <select
                value={team1?.id || ''}
                onChange={(e) => setTeam1(teams.find((t) => t.id === Number(e.target.value)))}
              ...
              <select
                value={team2?.id || ''}
                onChange={(e) => setTeam2(teams.find((t) => t.id === Number(e.target.value)))}
                className="glass-strong rounded-xl px-4 py-3 text-surface-900 dark:text-surface-100 font-medium cursor-pointer outline-none"
              >
                {teams.map((t) => (
                  <option key={t.id} value={t.id}>{t.name}</option>
            ...
            >
              <div className="flex items-center justify-center gap-6 lg:gap-12">
                <div className="flex-1">
                  <h2 className="text-2xl lg:text-3xl font-bold text-surface-900 dark:text-surface-100 mb-1">{team1?.name}</h2>
                  <p className="text-sm text-surface-500">{team1Stats.league}</p>
                </div>
                <span className="text-4xl lg:text-5xl font-black text-surface-300 dark:text-surface-600">VS</span>
                <div className="flex-1">
                  <h2 className="text-2xl lg:text-3xl font-bold text-surface-900 dark:text-surface-100 mb-1">{team2?.name}</h2>
                  <p className="text-sm text-surface-500">{team2Stats.league}</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              {[
                { label: 'Matches Played', v1: team1Stats.played, v2: team2Stats.played },
                { label: 'Wins', v1: team1Stats.wins, v2: team2Stats.wins },
                { label: 'Draws', v1: team1Stats.draws, v2: team2Stats.draws },
                { label: 'Losses', v1: team1Stats.losses, v2: team2Stats.losses, higher: false },
                { label: 'Goals For', v1: team1Stats.goalsFor, v2: team2Stats.goalsFor },
                { label: 'Goals Against', v1: team1Stats.goalsAgainst, v2: team2Stats.goalsAgainst, higher: false },
                { label: 'Clean Sheets', v1: team1Stats.cleanSheets, v2: team2Stats.cleanSheets },
              ].map((stat) => {
                const bars = compareBar(stat.v1, stat.v2, stat.higher !== false);
                return (
                  <div key={stat.label} className="py-3 border-b border-surface-100 dark:border-surface-800/50">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <p className="text-sm text-surface-500 mb-2">{team1?.name}</p>
                  <div className="flex justify-center gap-1.5">
                    {team1Stats.form.map((r: string, i: number) => (
                      <span key={i} className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${r === 'W' ? 'bg-success text-white' : r === 'D' ? 'bg-warning text-white' : 'bg-danger text-white'}`}>{r}</span>
                    ))}
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-sm text-surface-500 mb-2">{team2?.name}</p>
                  <div className="flex justify-center gap-1.5">
                    {team2Stats.form.map((r: string, i: number) => (
                      <span key={i} className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${r === 'W' ? 'bg-success text-white' : r === 'D' ? 'bg-warning text-white' : 'bg-danger text-white'}`}>{r}</span>
                    ))}
                  </div>
                </div>
